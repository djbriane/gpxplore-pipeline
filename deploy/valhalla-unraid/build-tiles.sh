#!/usr/bin/env bash
# Build a us-west Valhalla tile set on the Unraid NUC, then restart the serve container.
...
APPDATA="${APPDATA:-/mnt/user/appdata/valhalla}"        # mounted as /data in the container
...
PBF="$APPDATA/extract.osm.pbf"
SIDECAR="$APPDATA/surface_provenance.sqlite"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"  # so we can mount our own scripts
mkdir -p "$APPDATA"
log(){ printf '\n\033[1m=== %s ===\033[0m\n' "$*"; }
run(){ docker run --rm -v "$APPDATA":/data "$IMAGE" "$@"; }

# ---- 1. download extract (capture provenance) -----------------------------
if [[ "$REUSE_PBF" != "1" || ! -f "$PBF" ]]; then
  log "Downloading us-west extract"
  # -w prints the resolved (dated) Geofabrik filename, our provenance handle
  EFFECTIVE_URL=$(curl -fL "$EXTRACT_URL" -o "$PBF" -w '%{url_effective}')
else
  EFFECTIVE_URL="(reused existing $PBF)"
  log "Reusing existing extract"
fi
# Geofabrik resolves to e.g. us-west-260715.osm.pbf -> extract date YYMMDD
EXTRACT_DATE=$(printf '%s' "$EFFECTIVE_URL" | grep -oE '[0-9]{6}' | tail -1 || true)
EXTRACT_DATE="${EXTRACT_DATE:-unknown}"
...
run valhalla_build_config \
  ...
  > "$APPDATA/valhalla.json"

# ---- 3. optional enrichment DBs (before tiles) ----------------------------
if [[ "$BUILD_ADMINS" == "1" ]]; then
  log "valhalla_build_admins"; run valhalla_build_admins -c /data/valhalla.json /data/extract.osm.pbf
fi
if [[ "$BUILD_TIMEZONES" == "1" ]]; then
  ...
  docker run --rm -v "$APPDATA":/data "$IMAGE" valhalla_build_timezones > "$APPDATA/timezones.sqlite"
fi

# ---- 4. build tiles (timed) -----------------------------------------------
log "valhalla_build_tiles — the slow step (~20+ min for us-west)"
START=$(date +%s)
run valhalla_build_tiles -c /data/valhalla.json /data/extract.osm.pbf
...
if [[ "$BUILD_SIDECAR" == "1" ]]; then
  ...
  SIDECAR_OUT=$(docker run --rm \
    -v "$APPDATA":/data -v "$SCRIPT_DIR":/scripts:ro \
    "$PYOSMIUM_IMAGE" bash -c '
      ...
      python3 /scripts/build_surface_sidecar.py \
        /data/extract.osm.pbf /data/surface_provenance.sqlite \
...
TILE_BYTES=$(du -sb "$APPDATA/tiles" 2>/dev/null | cut -f1 || echo 0)
cat > "$APPDATA/tile_manifest.json" <<JSON
{
  "region": "us-west",
...
JSON
log "Manifest -> $APPDATA/tile_manifest.json"
cat "$APPDATA/tile_manifest.json"

# ---- 8. hot-swap: restart the serve container to load new tiles -----------
...
log "Done in $((BUILD_SECS/60))m $((BUILD_SECS%60))s. Tiles: $(du -sh "$APPDATA/tiles" 2>/dev/null | cut -f1)"
